...
export default function Diary() {
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      setCough(0); setWheeze(0); setSob(0); // keep notes
    } finally {
      setSaving(false);
    }
  }
        ...
        <section className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          {!isAuthenticated && !isLoading && (
            <div className="mb-4 text-sm text-slate-600">
              Optional sign-in: Log in to sync securely; otherwise entries stay on your device.
            </div>
          )}
          ...
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={saving}
              className="px-4 py-2 rounded-xl bg-indigo-600 text-white font-medium hover:bg-indigo-500 disabled:opacity-60"
            >
